...
| UDP payload với custom RDT | Excellent | B | 05 | `common/rdt_sender.py`, `rdt_receiver.py`, `RDTHeader.py` | `python3 -m pytest tests/test_rdt.py tests/test_rdt_fault_injection.py -q` → **45 passed in 67.09s** | Verified |
| Reliable transfer under loss/corruption | Excellent | B | 05 | `common/rdt_sender.py`, `common/rdt_receiver.py`, `tests/test_rdt_fault_injection.py` | Fault injection tests: packet loss, checksum corruption, ACK loss, cancel/abort recovery | Verified |
| Transfer lifecycle FIN/ABORT | Advanced/Excellent | B | 05 | `common/rdt_sender.py`, `common/rdt_receiver.py` | Protocol tests for FIN grace and abort cancellation | Verified |
