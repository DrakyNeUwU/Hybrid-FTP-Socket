...
|---|---|---|---|
| TCP control, parser, session, command lifecycle | In progress | A | MODE S/B/C done (see below); auth/STAT/HELP/STOU/framing remain with A |
| MODE S/B/C functional codec + integration | Done | A | Codec/command/transfer-manager/fault-injection/E2E tests; 256 passed full regression |
...
- Full regression after Role A MODE S/B/C: `python3 -m pytest -q` —
  **256 passed, 357 subtests in 167.08s** (was 205 passed/103.08s pre-MODE).
